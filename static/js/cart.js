window.onload = () => {
  ...
  async function updateCart(event) {
    ...
    let oldCartItemTotal = parseInt(cartItemTotalEl.innerHTML);
    let oldCartTotal = parseInt(cartTotalEl.innerHTML);
    let newCartItemTotal = quantity * price;
    let newCartTotal = oldCartTotal + (newCartItemTotal - oldCartItemTotal );
    cartItemTotalEl.innerHTML = newCartItemTotal;
    cartTotalEl.innerHTML = newCartTotal
  ...
  async function getCartDetails() {
    ...
    data.items.forEach((item) => {
      html += ` <div class="d-flex justify-content-around cart-item">
        ...
        <div class="d-flex justify-content-between align-items-center pt-2 px-4">
          ...
          <div>X</div>
          <input id="cartItem-${item.id}-${
        item.product__price
      }" type="number" class="form-control text-center cart-textbox" min="1" value="${
        item.quantity
      }"/>
          <div> = </div>
          <div> <span id="cartItemTotal-${item.id}"> ${
        item.product__price * item.quantity
      }</span> ${currency}</div>
        </div>
        <hr>
      `;
    ...
    <div>
      ...
      <div class="d-flex justify-content-end px-4 fs-4">
       <div class="">Total</div>
       <div class="mx-3"> = </div>
       <div> <span id="cartTotal">${total}</span> ${currency}</div>
      </div>
